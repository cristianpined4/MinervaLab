        ...
        <div class="flex-1 p-6 md:p-8 lg:p-10">
            <div class="max-w-7xl">
                ...
                <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <h1 class="text-3xl md:text-4xl font-bold text-white mb-2">Administrar mantenimiento</h1>
                        <p class="text-white/60">Gestión de periodos de mantenimiento de habitaciones</p>
                    </div>
                        ...
                        <table class="w-full">
                            <thead class="bg-white/5 border-b-2 border-white/10">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-bold text-white/60 uppercase tracking-wider">Habitación</th>
                            ...
                            <tbody class="divide-y divide-white/20">
                                @foreach ($data as $row)
                                    <tr class="hover:bg-white/5 transition-colors">
                                        <td class="px-6 py-4">
                                            <span class="text-sm font-medium text-white">@dateTimeFormat($row->starts_at)</span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <span class="text-sm font-medium text-white">@dateTimeFormat($row->ends_at)</span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <span class="text-sm font-medium text-white">{{ $row->room->description ?? 'N/A' }}</span>
                                        </td>
                                        <td class="px-6 py-4">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <div class="flex justify-center gap-2">
                                                <button class="p-2 bg-blue-600 hover:bg-blue-700 text-white inline-flex items-center justify-center rounded-lg transition-colors" wire:click="abrirModal({{ $row->id }})">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                            d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                                    </svg>
                                                </button>
                                                <button class="p-2 bg-red-600 hover:bg-red-700 text-white inline-flex items-center justify-center rounded-lg transition-colors" wire:click="confirmarEliminar({{ $row->id }})">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
